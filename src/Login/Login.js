...
function Login(props) {
    ...
    const handleClose = (event, reason) => {
        ...
        setOpen(false);
        history.push({pathname: "./confirm_otp", generated_otp: generated_otp, entered_mobile_number: entered_mobile_number })
    };
    ...
    async function LoginSubmitForm(e) {
        ...
        if (result.info === "success") {
            setgenerated_otp(result.generated_otp)
            setentered_mobile_number(result.entered_mobile_number)
            handleClick()
        } else {
            ...
            <div className='container-md text-center text-light pt-5'>
                ...
                <form className='p-3 p-md-5' onSubmit={LoginSubmitForm}>
                    <FormControl variant="standard">
                        ...
                        <Input style={{ color: "white", fontSize: "20px" }} value={LoginMobileNumber} onChange={(e) => setLoginMobileNumber(e.target.value)} required inputProps={{ maxLength: 10, minLength: 10 }} autoFocus startAdornment={<InputAdornment position="start"><span style={{ color: "white", fontSize: "20px" }}>+91</span></InputAdornment>} />
