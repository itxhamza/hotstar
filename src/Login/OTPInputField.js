...
export default class OTPInputField extends Component {
    ...
    async FormSubmitConfirmOTP(e) {
        e.preventDefault()
        let generated_otp = this.props.generated_otp
        let entered_mobile_number = this.props.entered_mobile_number
        let data = { generated_otp, entered_mobile_number }
        let result = await fetch(`${this.props.BaseUrl}/confirm_otp_process/${this.state.generatedOTP}`, {
            method: "POST",
        ...
        if (result.info === "success") {
            // console.log(result.UserDetail.mobile)
            this.props.dispatch(LoginDetailsSave(result.UserDetail.id, result.UserDetail.mobile, result.UserDetail.create_at, result.UserDetail.token, result.UserDetail.jwt_token))
        } else {
